...
fun fft(entries: List<Entry>): List<Double> {
    val cutoffFrequency = 5.0F
    val sampleRate = 12.0F
    ...
    //val paddedEntries = entries + List(64 - entries.size) { Entry(0.0F, 0.0F) }
    val yValues = paddedEntries.map { it.y }

    // Remove the mean from yValues
    //val yValuesWithoutMean = removeMean(yValues)
    ...
    //val LowPassfilteredData = lowPassFilter(yValuesWindowed, 3.0, 12) //cutofffrequency = 6Hz. sampling rate = 12
    //Apply Lowpass filter on frequency domain
    val lowPassFilter = createFrequencyDomainLowPassFilter(cutoffFrequency, sampleRate, fftResult.size)
    val result = applyFrequencyDomainLowPassFilter(fftResult, lowPassFilter)
...
fun detectActivity(magnitudes: List<Double>, samplingRate: Float): String {
    ...
    val maxFrequencyIndex = magnitudes.indices.drop(1).maxByOrNull { magnitudes[it] } ?: 5
    ...
    return when {
        ...
        dominantFrequency in 1.6..2.8 -> "WALKING"
        else -> "RUNNING"
    }
}
